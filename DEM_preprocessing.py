...
# =================================
# 3. Resample elevation to lower resolution

# Read elevation and downsample
with rasterio.open("data/ASTERGDEM3/Processed/Elevation_raw.tif") as dataset:
    elevation_low = dataset.read(
        out_shape=(dataset.count, dataset.height // 4, dataset.width // 4),
...
out_meta.update({"driver": "GTiff",
                 ...
                 "transform": Affine(0.000277777777777778 * 4, 0.0, 31.9998611111111, 
                                     0.0, -0.000277777777777778 * 4, 47.0001388888889),
                 "crs": "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"
                 }
    )
...
# Write the mosaic raster to disk
# First, specify outputfile path
out_tif = "data/ASTERGDEM3/Processed/Elevation_lowres.tif"
with rasterio.open(out_tif, "w", **out_meta) as dest:
...
elevation_raw = rd.LoadGDAL("data/ASTERGDEM3/Processed/Elevation_lowres.tif", no_data = -9999)
...
# Export as tif
rd.SaveGDAL("data/ASTERGDEM3/Processed/Slope.tif", slope)
...
# Export as tif
rd.SaveGDAL("data/ASTERGDEM3/Processed/Aspect.tif", aspect)
